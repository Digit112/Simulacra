...
public:
	...
	// Same as paint() but add values instead of averaging them.
	void add(pattern2 p, int x = 0, int y = 0);
	
	// Produce in this pattern 1s corresponding to values within the specified range in the supplied pattern.
	void range(pattern2 p, float min, float max, int x = 0, int y = 0);
	
	// Applies the logistic function to all values.
	void logistic(float s, float o);
	
	// Scale all the values in this pattern such that the lowest becomes 0 and the highest becomes 1
	void fit_values();
	
	// Roucd all values below zero up to zero and round all values above 1 down to 1.
	void clamp();
