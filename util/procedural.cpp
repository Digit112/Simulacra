...
void pattern2::range(pattern2 p, float min, float max, int x, int y) {
	int Mx = x + p.width;
	if (width < Mx) Mx = width;
	int My = y + p.height;
	if (height < My) My = height;
	...
	for (y0 = 0, y1 = y; y1 < My; y1++) {
		...
		for (x0 = 0, x1 = x; x1 < Mx; x1++) {
			data[rowc1+x1] = (p.data[rowc0+x0] > min && p.data[rowc0+x0] < max) ? 1 : 0;
			x0++;
		}
		y0++;
	}
...
void pattern2::logistic(float s, float o) {
	for (int i = 0; i < width*height; i++) {
		data[i] = 1/(1+pow(2.718282, -s*(data[i]-o)));
	}
}
